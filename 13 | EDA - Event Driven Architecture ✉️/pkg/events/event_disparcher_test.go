...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Resgister() {
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Resgister_WithSameHandler() {
	...
	err = suite.eventDispatcher.Register(suite.event.name, &suite.handler)
	suite.Equal(err.Error(), "handler already registered")
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.name]))
}
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Dispatch_Remove() {
	// Event 1
